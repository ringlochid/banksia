...
from banksia.workflows.authoring_contracts import WorkflowSearchItem
...
async def cmd_task_start(args: argparse.Namespace) -> int:
    ...
    if sources:
        ...
        click.echo(f"Workflow: {response.workflow} revision {response.workflow_revision}")
...
async def _get_published_workflows(
    ...
    try:
        items = [WorkflowSearchItem.model_validate(item) for item in response.json()]
    ...
    return tuple(item for item in items if item.published_revision_no is not None)
...
async def _post_task_start(
    ...
    try:
        return TaskStartResponse.model_validate(response.json())
    except (ValueError, ValidationError) as exc:
        raise TaskStartCliError(
