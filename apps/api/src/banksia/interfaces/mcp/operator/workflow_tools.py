...
def _register_workflow_search(server: FastMCP) -> None:
    search_teaching = read_only_tool_teaching(
        ...
        summary="Search published Workflows and active controller-owned drafts.",
        details=("Use workflow_get for the complete selected draft and revision history.",),
    )
    ...
    async def workflow_search(query: str | None = None, limit: int = 50) -> WorkflowSearchResponse:
        if not 1 <= limit <= 256:
            raise ValueError("limit must be between 1 and 256")
        items = await session_operations.read_session_operation(
            lambda session: search_workflow_catalog(session, query=query)
        )
        return WorkflowSearchResponse(items=items[:limit])
...
def _register_workflow_get(server: FastMCP) -> None:
    ...
    async def workflow_get(
        ...
        if not 1 <= revision_limit <= 256:
            raise ValueError("revision_limit must be between 1 and 256")
