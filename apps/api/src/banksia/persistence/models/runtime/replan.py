...
class ReplanTransitionModel(RuntimeBase):
    """Durable bridge between an accepted replan and its fresh Dispatch."""
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            name="fk_replan_transitions_source_flow_revision",
            deferrable=True,
            initially="DEFERRED",
        ),
        ForeignKeyConstraint(
            ["flow_id", "successor_flow_revision_id"],
            ["flow_revisions.flow_id", "flow_revisions.flow_revision_id"],
            name="fk_replan_transitions_successor_flow_revision",
            deferrable=True,
            initially="DEFERRED",
        ),
        ForeignKeyConstraint(
            ...
            [
                "flow_revisions.flow_id",
                "flow_revisions.flow_revision_id",
                "flow_revisions.parent_flow_revision_id",
            ],
            name="fk_replan_transitions_successor_flow_parent",
            deferrable=True,
            initially="DEFERRED",
        ),
    )
    ...
    successor_flow_revision_id: Mapped[str] = mapped_column(String(255))
    ...
    flow: Mapped[FlowModel] = relationship("FlowModel", foreign_keys=[flow_id], lazy="raise")
    assignment: Mapped[AssignmentModel] = relationship(
        "AssignmentModel",
        foreign_keys=[task_id, flow_id, assignment_id],
        lazy="raise",
        viewonly=True,
    )
    ...
    successor_flow_revision: Mapped[FlowRevisionModel] = relationship(
        "FlowRevisionModel",
        foreign_keys=[flow_id, successor_flow_revision_id],
        lazy="raise",
        viewonly=True,
    )
