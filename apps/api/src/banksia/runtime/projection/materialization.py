...
from banksia.runtime.projection.contracts import (
    ArtifactIndexEntry,
...
from banksia.runtime.task_root.writes import (
    ...
    for node in payload.nodes:
        lines.extend(
            [
                ...
                f"- Team revision: `{node.team_revision_id}`",
                f"- Member: `{node.member_id}`",
                f"- Member configuration: `{node.member_configuration_id}`",
                f"- Branch basis: `{node.member_branch_basis_id}`",
