...
from banksia.runtime.post_commit import ReplanCommitted
...
async def continue_committed_replan(
    ...
    *,
    ...
    return await open_ordinary_successor(
        ...
        should_resume_flow=expected_flow_status == "paused",
...
async def _ensure_manifest_current(
    ...
    try:
        projected = await project_workflow_manifest(
            ...
            WorkflowManifestProjection(
                ...
                active_flow_revision_id=transition.successor_flow_revision_id,
            ),
        )
    ...
    if not projected:
        await _record_manifest_failure(
            ...
            detail="The successor Flow was no longer current during manifest projection.",
        )
        return False
