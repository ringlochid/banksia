...
async def search_workflow_catalog(
    ...
    *,
    ...
    published = await search_workflows(session)
    drafts = await list_active_workflow_drafts(session)
    published_by_id = {item.workflow_id: item for item in published}
    drafts_by_id = {item.workflow_id: item for item in drafts}
    normalized_query = query.casefold().strip() if query else None
    items: list[WorkflowSearchItem] = []
    for workflow_id in sorted(published_by_id.keys() | drafts_by_id.keys()):
        published_item = published_by_id.get(workflow_id)
        draft = drafts_by_id.get(workflow_id)
        description = (
            draft.workflow.description
            if draft is not None
            else published_item.description
            if published_item is not None
            else ""
        )
        if normalized_query and normalized_query not in f"{workflow_id} {description}".casefold():
            continue
        items.append(
            WorkflowSearchItem(
                ...
                published_revision_no=(
                    published_item.revision_no if published_item is not None else None
                ),
                provenance=published_item.provenance if published_item is not None else None,
                draft_id=draft.draft_id if draft is not None else None,
                draft_etag=draft.etag if draft is not None else None,
            )
        )
    return tuple(items)
...
async def read_workflow_catalog_entry(
    ...
    *,
    ...
    revision_history = (
        await list_workflow_revisions(session, workflow_id=workflow_id)
        if should_include_revisions and published is not None
        else ()
    )
    if revision_limit is not None:
        revision_history = revision_history[:revision_limit]
    return WorkflowGetResponse(
        workflow_id=workflow_id,
        published=(map_workflow_published_readback(published) if published is not None else None),
        revisions=tuple(map_workflow_revision_readback(item) for item in revision_history),
        active_draft=draft,
    )
