...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            "dispatch-flow-node-snapshot",
            lambda connection, ids: _mismatch_dispatch_flow_node(connection, ids),
        ),
        (
...
async def test_postgresql_rejects_wp02_cross_record_and_capability_widening() -> None:
    ...
    cases: tuple[tuple[str, Mutation], ...] = (
        ...
        ("dispatch-node", _mismatch_dispatch_flow_node),
...
def _mismatch_dispatch_flow_node(connection: Connection, ids: RuntimeIds) -> None:
    dispatches = RuntimeBase.metadata.tables["dispatch_turns"]
    connection.execute(
        dispatches.update()
        .where(dispatches.c.dispatch_id == ids.current_dispatch_id)
        .values(flow_node_id=ids.child_node_id)
    )
