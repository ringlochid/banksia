...
async def test_bootstrap_exposes_exact_portable_starter_workflow_set(tmp_path: Path) -> None:
    ...
    assert tuple(item.workflow_id for item in workflows) == (
        "autonomous-delivery",
        "evidence-research",
        "reviewed-delivery",
    )
    assert all(item.provenance is WorkflowProvenance.STARTER_SEED for item in workflows)
...
async def test_publish_is_immutable_and_seed_refresh_never_replaces_user_current(
    ...
    assert tuple(item.revision_no for item in history) == (3, 2, 1)
