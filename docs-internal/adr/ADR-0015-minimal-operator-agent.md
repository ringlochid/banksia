...
Banksia implements Operator as one small, separate provider-backed agent over existing product services. Claude and Codex are supported adapters. Each turn receives the same eighteen Banksia product operations and returns one provider-native typed `message | ask_user` result.
