# Accepted decisions

Status: Reference

This surface owns durable accepted decisions for Banksia and its imported AutoClaw baseline. ADR-0013 establishes the Banksia clean-break authority; older ADRs remain useful rationale only where the versionless Banksia owners do not supersede their product or implementation shape.

## Start here

- [ADR-0001 controller-first relational runtime truth](ADR-0001-controller-first-relational-runtime-truth.md)
- [ADR-0002 deterministic compiler and immutable compiled plans](ADR-0002-deterministic-compiler-and-immutable-compiled-plans.md)
- [ADR-0003 parent-owned execution tree and boundary advancement](ADR-0003-parent-owned-execution-tree-and-boundary-advancement.md)
- [ADR-0004 OpenClaw adapter normalization and worker transport boundary](ADR-0004-openclaw-adapter-normalization-and-worker-transport-boundary.md)
- [ADR-0005 task-owned directories and runtime-generated projections](ADR-0005-task-owned-roots-and-runtime-generated-projections.md)
- [ADR-0006 revision-safe replan and adopt](ADR-0006-revision-safe-replan-and-adopt.md)
- [ADR-0007 MCP-anchored local runtime and minimal provider control](ADR-0007-mcp-anchored-local-runtime-and-minimal-provider-control.md)
- [ADR-0008 task-relative MCP reads and reduced task root](ADR-0008-task-relative-mcp-reads-and-reduced-task-root.md)
- [ADR-0009 exact-source runtime control](ADR-0009-exact-source-runtime-control.md)
- [ADR-0010 dispatch-scoped managed Node MCP authority](ADR-0010-dispatch-scoped-managed-node-mcp-authority.md)
- [ADR-0011 provider routing, defaults, and capability resolution](ADR-0011-provider-routing-defaults-and-capability-resolution.md)
- [ADR-0012 loopback control plane without an operator API key](ADR-0012-loopback-control-plane-without-operator-api-key.md)
- [ADR-0013 Banksia target and clean break](ADR-0013-banksia-target-and-clean-break.md)
