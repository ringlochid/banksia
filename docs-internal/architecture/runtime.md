...
This page owns the shipped controller record model, transition rules, currentness, Delegation Wave semantics, replan behavior, waits, and exact Task Result.
...
request:
  ...
  items: # 1..3, stable request-local IDs
    - id: interface-boundary
      prompt: Which public interface should the team preserve?
      options: # 2..3 when used
        - id: preserve-current
          title: Preserve the current interface
...
TeamRevision and TeamRevisionMember are the sole immutable structural history and selection records. A DelegationWave pins the parent Dispatch's TeamRevision and parent Member. Each ordered Wave member pins one exact direct-child TeamRevisionMember selection. Replan advances only `Task.current_team_revision_id`; it has no second structural head or dual compare-and-swap.
