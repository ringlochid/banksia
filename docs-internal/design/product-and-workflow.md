...
The maintained [Workflow schema](appendices/workflow-definition.schema.yaml) is a JSON Schema document serialized as YAML. It validates the JSON-compatible value created by either a strict JSON parser or a strict YAML parser.
...
Workflow and Task authoring contain no Role, Policy, Skill, Extension, generic capabilities, limits, arbitrary tools, external MCP configuration, steps, stages, handoffs, edges, dependencies, loop/batch mode, criteria, consume, produce, expected-output declaration, slots, transient refs, hashes, semantic versions, current pointers, controller-owned Artifact resources, arbitrary provider options, or standalone network field. The lowercase `artifacts/` workspace directory is only a loose-file convention and does not contradict this resource-model exclusion.
