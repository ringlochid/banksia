...
9. replace FlowNode's mutable `state` and `current_assignment_id` with Assignment/Attempt truth: a partial unique constraint permits at most one open Assignment for one `(task_id, member_id)`, while member work state is derived from that Assignment, its current Attempt/wait, and its terminal history; and
