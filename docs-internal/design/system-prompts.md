...
This page owns the exact target system-prompt contract for Task members. The controller maintains these assets; Workflow authors do not copy them into a Workflow `note` or Member `instruction`.
...
The target follows these provider-neutral rules:
...
8. **Change prompts through evaluation.** Golden rendering, behavior scenarios, and supported provider/model comparisons gate additions, removals, and wording changes. Prompt length has no arbitrary target, but every paragraph must earn its context cost.
...
The following text is the implementation target. Wording may change only with prompt-evaluation evidence and an owning canon update.
...
The target imports a consistent pattern from current primary sources:
...
The Operator is a separate control-plane agent and needs its own later prompt. This page owns only Task member prompts.
