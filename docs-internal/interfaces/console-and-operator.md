...
| `GET /api/tasks/{task_id}` | Read canonical `TaskView`, including current team, plan, attention, legal actions, bounded Activity and Command previews, and exact Result. | `TaskView` | Console, Operator `task_get` |
...
| `GET /api/tasks/{task_id}/command-runs` | Page product-safe managed Actions from an opaque cursor. | `CommandRunPage` | Console |
...
When a paused Task's original workspace or accepted Task root is unavailable, `TaskView` includes a plain-language workspace attention item, omits Resume, and retains Cancel. If the root becomes accessible again, the attention item says so and Resume returns, but no automatic continuation occurs. The Console renders those controller-returned facts; it does not probe the filesystem or infer workspace health itself.
