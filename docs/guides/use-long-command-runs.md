...
The worker's policy must allow command runs. The worker submits the command through its node tool. Banksia commits the wait, launches and supervises the process asynchronously, records terminal state, and signals continuation from that exact run.
