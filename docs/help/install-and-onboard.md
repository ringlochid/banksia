...
The interactive setup path is `banksia init` followed by `banksia setup`. The first command confirms local state. The second asks for the primary/default provider, checks it, offers supported login, and asks about additional providers.

## `banksia` is not found
...
Rerun `banksia init` and choose the default `keep` action to verify existing config and database state. Use `banksia init --help` for explicit path and bind options. Do not select replacement or use `--force` until you understand which existing config it would replace. `init` does not reset an old database schema.
...
Banksia will not fall back to another configured provider silently.
