...
Confirm the provider is installed and authenticated through its supported native flow. Banksia attaches the managed MCP connection dynamically for each dispatch; do not add it to global or project provider configuration.
...
Reconfigure the Banksia provider record when its Gateway URL or profile changes:
