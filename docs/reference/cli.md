...
| --- | --- |
| `oms init` | Prepare local controller state, then offer first-run Task-provider and optional Operator setup. |
| `oms setup` | Reopen the settings hub for Task providers, Operator, or the default workspace. |
| `oms status` | Read passive configuration and local provider status. |
| `oms config` | `path`, `show` — print the selected configuration path or effective redacted settings. |
| `oms providers` | `list`, `status`, `configure`, `login`, `logout`, `check`, `set-default` — inspect and operate provider routes. |
| `oms operator` | `setup`, `status`, `disable` — configure or inspect the separate Operator without changing Task-provider routing. |
| `oms workflow` | `import`, `export` — import one draft or export a published Workflow revision. |
| `oms task start` | Start one Task interactively or from strict JSON. |
| `oms serve` | Run the loopback application in the foreground. |
| `oms service` | `render`, `install`, `start`, `stop`, `restart`, `status`, `logs`, `uninstall` — render or operate this host's per-user background service. |
| `oms db` | `upgrade`, `reset` — create or verify exact storage, or destructively reset it. |

Bare `oms`, `oms status`, `oms providers status`, and `oms operator status` are passive. They do not initialize or upgrade storage, start a service, contact a provider, repair work, or write configuration.

Most stateful commands accept `--config PATH`. When omitted, configuration selection follows `OMS_CONFIG` and then the platform default. See [Configuration](configuration.md).
...
SQLite is the default. PostgreSQL needs the `oh-my-subagents[postgres]` installation extra and an explicit `--database-url`; see [Database configuration](configuration.md#database).
...
Initialization never guesses how to change a nonexact existing database. When a supported older Oh My Subagents schema is found, preserve it and run the upgrade with the same configuration:

```bash
oms db upgrade
```

An applied upgrade reports a private backup path. SQLite uses an adjacent integrity-checked online backup; PostgreSQL writes a nonempty custom-format dump of the dedicated Oh My Subagents schema under `paths.data_dir/database-backups/` and requires a compatible `pg_dump` client. Unknown or locally changed schemas are refused.

`oms db reset` is still destructive, but it now creates the same backup before deleting controller-owned Task roots or replacing an existing database/schema. The command reports `backup_path` in JSON and prints it in ordinary output. If backup creation fails, reset stops before deletion. Preserve that file until you have verified the replacement and no longer need rollback.
...
oms providers configure codex --model gpt-5.6 --effort high
...
```bash
oms workflow import --file workflow.yaml
oms workflow import --file workflow.json
oms workflow import --file - --format yaml < workflow.yaml
```
...
```bash
oms workflow import --file workflow.yaml --etag '<current-etag>'
```
...
```bash
oms workflow export production-feature-delivery \
...
oms workflow export production-feature-delivery --format yaml
```
...
Oh My Subagents selects one native current-user manager:
...
`oms serve` remains the portable foreground path. A native service implementation on a host does not by itself establish full platform readiness; supported-platform claims require the installed-wheel filesystem, private-path, Command Run, reset, and lifecycle gates.
