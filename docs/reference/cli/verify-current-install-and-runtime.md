...
Use `banksia providers status` for passive provider configuration and `banksia providers check <provider>` for an explicit live diagnostic. Do not use task start, database reset, provider login, or service restart as a status check.
