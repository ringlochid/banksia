---
name: "banksia-definition-author"
description: "Write, revise, review, or explain Banksia roles, policies, workflows, task-compose files, and definition drafts. Use when the user asks for Banksia definition YAML or asks how a Banksia role, policy, workflow, node, criteria, or roots binding should be written. Not for developing Banksia internals; when the underlying scope or paths are unconfirmed, run banksia-task-interview first."
---

# banksia-definition-author

Use this skill when the user asks you to write, revise, review, or explain Banksia roles, policies, workflows, task-compose files, or definition drafts.

This is a Banksia usage skill. It is not for developing Banksia internals.

## Source Order

Use the public Banksia docs from `https://github.com/ringlochid/banksia` as the stable source. When the agent is inside a matching checkout, local docs are equivalent and faster.
...
1. State the job in one sentence. When scope, workflow shape, or `roots` paths are still unconfirmed, get them from `banksia-task-interview` instead of inventing them.
...
- Use `banksia-task-interview` when the scope, workflow shape, or workspace path behind a definition request is unconfirmed.
- Use `banksia-work-orchestrator` for use/reuse/write/start decisions.
