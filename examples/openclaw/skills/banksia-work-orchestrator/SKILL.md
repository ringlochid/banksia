---
name: "banksia-work-orchestrator"
description: "Turn a 'use Banksia to do X' request into launched Banksia work: decide whether Banksia fits, pick or draft the right workflow, prepare task-compose, and launch or hand off. Use whenever the user asks to use Banksia to build, make, research, fix, plan, coordinate, or run something — that means launching Banksia-orchestrated work, not integrating Banksia into an app and not modifying Banksia source. Run banksia-task-interview first when scope, workflow shape, or roots paths are unconfirmed."
---

# banksia-work-orchestrator

Use this skill when the user asks you to use Banksia to coordinate, launch, or supervise a task.

This is the front door for Banksia usage. Keep it practical: choose whether to stay in chat/tools, reuse definitions, write definitions, draft task-compose, start a task, or hand off to runtime operation.

## Read The Request Correctly

"Use Banksia to build me an MVP" means launch a Banksia task that builds the MVP. It never means adding Banksia as a dependency of the user's app, wiring Banksia into a stack, or editing Banksia source code. Treat a request as integration or internals work only when the user says so explicitly ("integrate Banksia into my service", "fix this Banksia bug") — and then say you are leaving the Banksia usage lane. When the sentence genuinely supports two readings, ask one direct question before doing anything.

## Core Model

Banksia is useful when work benefits from durable roles, explicit evidence, checkpoints, artifacts, human decisions, long command runs, recovery, or auditable closure.
...
2. Decide whether Banksia adds value. Stay local for small one-shot tasks.
3. For new or ambiguous work, run `banksia-task-interview` first: it confirms intent, scope, workflow shape, and `roots` path bindings, and produces the launch brief this skill executes.
4. Read only the current docs/reference needed for the decision, using `https://github.com/ringlochid/banksia` as the public source or a matching local checkout.
...
9. After start, report the task id and stop; route later inspection/control to `banksia-runtime-operator` when the user asks.
...
`banksia-task-interview` owns the structured intake for new work. Use this pass only for residual questions that surface after the interview, or for small follow-up launches where a full interview is overkill.
...
Default after start: confirm the task is running with one `get_runtime_task` readback, report the task id, task-compose path, and where results will land, then stop. Do not keep polling, watching, or resolving waits on your own — the user will ask for status or results later, and those reads belong to `banksia-runtime-operator`.
...
- Use `banksia-task-interview` first for new or ambiguous work: intent, scope, workflow shape, and `roots` confirmation.
- Use `banksia-definition-author` for role/policy/workflow/task-compose YAML.
- Use `banksia-runtime-operator` after a task starts.
