...
async def _fetch_workflow_projection_page(
    ...
    async with session_factory() as session:
        statement = (
            select(TaskModel)
            .where(TaskModel.current_team_revision_id.is_not(None))
    ...
    signals = tuple(
        WorkflowManifestProjection(row.task_id, row.current_team_revision_id)
        for row in rows
        if row.current_team_revision_id is not None
    )
    return StartupAuditPage(
        signals,
        rows[-1].task_id if len(rows) == page_size else None,
    )
