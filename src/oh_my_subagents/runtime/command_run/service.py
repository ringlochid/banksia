...
from oh_my_subagents.runtime.contracts.command_runs import CommandRunTerminalState
from oh_my_subagents.runtime.contracts.operation_failure import OperationFailureCode
from oh_my_subagents.runtime.errors import RuntimeOperationError, missing_resource_error
...
from oh_my_subagents.runtime.workspace.regular_files import read_workspace_regular_file_range
...
async def read_command_run_log(
    ...
    *,
    ...
    should_preserve_utf8_boundaries: bool = False,
) -> CommandRunLogReadResponse:
    _validate_command_log_range(offset=offset, byte_limit=byte_limit)
    source = await _command_run_for_task(session, task_id=task_id, run_id=run_id)
    if source is None:
        raise missing_resource_error(f"unknown command run '{run_id}' for task '{task_id}'")
    ...
    try:
        current = await asyncio.to_thread(
            ...
            source.output_path,
            offset=offset,
            byte_limit=byte_limit + 3 if should_preserve_utf8_boundaries else byte_limit,
        )
    except FileNotFoundError:
        return _unavailable_command_log_response(
            source,
            task_id=task_id,
            offset=offset,
            is_missing=True,
    ...
    actual_offset = min(offset, current.file_size)
    if should_preserve_utf8_boundaries:
        content, bytes_read = _decode_utf8_page(
            current.payload,
            byte_limit=byte_limit,
            is_end_of_file=actual_offset + len(current.payload) >= current.file_size,
        )
        next_offset = (
            actual_offset + bytes_read if actual_offset + bytes_read < current.file_size else None
        )
    else:
...
async def request_command_run_cancellation(
    ...
    *,
    ...
    is_cancelled_task_allowed: bool = False,
    ...
    current_wait = _current_command_wait_exists(
        source,
        is_cancelled_task_allowed=is_cancelled_task_allowed,
    )
...
def _validate_command_log_range(*, offset: int, byte_limit: int) -> None:
    if offset < 0:
...
def _unavailable_command_log_response(
...
def _decode_utf8_page(
    payload: bytes,
    *,
    byte_limit: int,
    is_end_of_file: bool,
) -> tuple[str, int]:
    """Decode a page without advancing a generated cursor into a UTF-8 sequence."""

    target = min(byte_limit, len(payload))
    if target == 0:
        return "", 0
    ...
    content = decoder.decode(
        payload[:end],
        final=is_end_of_file and end == len(payload),
    )
    buffered, _flag = decoder.getstate()
    consumed = end - len(buffered)
    while consumed == 0 and end < len(payload):
        end += 1
        content += decoder.decode(
            payload[end - 1 : end],
            final=is_end_of_file and end == len(payload),
        ...
        buffered, _flag = decoder.getstate()
        consumed = end - len(buffered)
    if consumed == 0 and is_end_of_file:
...
def _current_command_wait_exists(
    ...
    *,
    ...
    return exists().where(
        ...
        TaskModel.status.in_(task_statuses),
    ) & exists().where(
