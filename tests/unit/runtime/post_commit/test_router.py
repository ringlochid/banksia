...
async def test_router_runs_handlers_concurrently_with_fresh_sessions() -> None:
    ...
    async def handle_task_start(
        ...
        assert signal.task_id in {"task.alpha", "task.beta"}
    ...
    router.register(TaskStartCommitted, handle_task_start)

    async with router:
        assert router.publish(TaskStartCommitted("task.alpha")) is True
        assert router.publish(TaskStartCommitted("task.beta")) is True
...
async def test_router_isolates_handler_failure_and_keeps_routing() -> None:
    ...
    async def handle_task_start(
        ...
        del session
        if signal.task_id == "task.failure":
    ...
    async with router:
        assert router.publish(TaskStartCommitted("task.failure")) is True
        assert router.publish(TaskStartCommitted("task.success")) is True
...
async def test_router_rejects_unregistered_and_full_queue_visibly() -> None:
    ...
    async with router:
        ...
        assert router.publish(TaskStartCommitted("task.queued")) is True
        assert router.publish(TaskStartCommitted("task.rejected")) is False
...
async def test_startup_publication_waits_only_for_queue_capacity() -> None:
    ...
    async with router:
        assert router.publish(TaskStartCommitted("task.first")) is True
        await asyncio.wait_for(handler_started.wait(), timeout=1)

        assert await asyncio.wait_for(
            router.publish_startup(TaskStartCommitted("task.second")),
            timeout=1,
        )
        release_handler.set()
...
async def test_router_context_exit_cancels_disposable_handler_work() -> None:
    ...
    assert router.publish(TaskStartCommitted("task.after-shutdown")) is False
