...
def test_style_audit_flags_oh_my_subagents_modules_that_import_app_outside_approved_shims(
    tmp_path: Path,
) -> None:
    oh_my_subagents_root = tmp_path / "oh_my_subagents"
    app_root = tmp_path / "app"
    settings = build_style_audit_settings(tmp_path, scan_roots=(oh_my_subagents_root, app_root))
    audit = load_style_audit_namespace()

    write_python_module(app_root / "runtime" / "owner.py", "VALUE = 1\n")
    write_python_module(
        oh_my_subagents_root / "consumer.py",
    ...
    assert findings[0].path == oh_my_subagents_root / "consumer.py"
    assert findings[0].owner_family == "oh_my_subagents"
    assert findings[0].violated_rule == "oh_my_subagents-consumer-imports-app-owner"
...
def test_style_audit_allows_approved_shim_import_direction_exceptions(
    ...
    oh_my_subagents_root = tmp_path / "oh_my_subagents"
    app_root = tmp_path / "app"
    consumer_path = oh_my_subagents_root / "cli.py"
    settings = replace(
        build_style_audit_settings(tmp_path, scan_roots=(oh_my_subagents_root, app_root)),
...
def test_style_audit_allows_legacy_app_shells_that_import_canonical_oh_my_subagents_owners(
    tmp_path: Path,
) -> None:
    app_root = tmp_path / "app"
    shell_path = app_root / "service_managers" / "base.py"
    ...
    write_python_module(
        shell_path,
        "from oh_my_subagents.platform.managed_services.contracts import ManagedServiceManager\n",
    )
...
def test_style_audit_flags_src_oh_my_subagents_modules_that_import_legacy_oh_my_subagents_owner(
    tmp_path: Path,
) -> None:
    ...
    write_python_module(
        ...
        "from oh_my_subagents.legacy_only import VALUE\n",
    )
    ...
    assert findings[0].violated_rule == "src-oh_my_subagents-consumer-imports-legacy-owner"


def test_style_audit_flags_src_oh_my_subagents_modules_that_import_legacy_owner_outside_scan_root(
    tmp_path: Path,
) -> None:
    src_root = tmp_path / "src" / "oh_my_subagents"
    legacy_root = tmp_path / "oh_my_subagents"
...
def test_style_audit_allows_canonical_import_with_duplicate_name(
    tmp_path: Path,
) -> None:
    src_root = tmp_path / "src" / "oh_my_subagents"
    legacy_root = tmp_path / "oh_my_subagents"
    ...
    write_python_module(src_root / "consumer.py", "from oh_my_subagents.common import VALUE\n")
