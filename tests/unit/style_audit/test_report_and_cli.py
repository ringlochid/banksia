...
def test_render_audit_report_includes_style_sections(tmp_path: Path) -> None:
...
def test_cli_main_passes_explicit_scan_roots(
    ...
    assert audit.cli.main(["--scan-root", "app"]) == 0
...
def test_cli_main_accepts_python_file_scan_roots(
    ...
    audit = load_style_audit_namespace()
    scan_root_argument = "tests/unit/style_audit/test_report_and_cli.py"
